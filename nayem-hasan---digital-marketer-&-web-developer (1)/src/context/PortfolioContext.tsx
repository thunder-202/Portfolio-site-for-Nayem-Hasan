...
export const PortfolioProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Safe fetch helper that guarantees proper JSON parsing and handles non-JSON/HTML error pages without crashing
  const safeJsonFetch = useCallback(async <T = any>(url: string, options?: RequestInit): Promise<{ ok: boolean; status: number; data: T | null; error?: string }> => {
    try {
      ...
      // If server returned non-JSON (e.g. 404/500 HTML or plain text)
      const errorMsg = res.status === 404
        ? 'API endpoint not reached (404)'
        : res.status === 401
        ? 'Invalid admin credentials or unauthorized'
        : res.status >= 500
  ...
  const refreshAllData = useCallback(async () => {
    try {
      const [resContent, resProjects, resCategories, resSettings, resSeo] = await Promise.all([
        safeJsonFetch<HomepageContent>('/api/content'),
        safeJsonFetch<ProjectCaseStudy[]>('/api/projects'),
        safeJsonFetch<CategoryDefinition[]>('/api/categories'),
        safeJsonFetch<WebsiteSettings>('/api/settings'),
        safeJsonFetch<SeoSettings>('/api/seo')
      ]);

      if (resContent.ok && resContent.data) {
        setContent(resContent.data);
        setDraftContent(resContent.data);
      }
      if (resProjects.ok && Array.isArray(resProjects.data) && resProjects.data.length > 0) {
        setProjects(resProjects.data);
        try {
          localStorage.setItem('nayem_portfolio_projects', JSON.stringify(resProjects.data));
        } catch (e) {
          // ignore
        }
      }
      if (resCategories.ok && Array.isArray(resCategories.data)) setCategories(resCategories.data);
      if (resSettings.ok && resSettings.data) setSettings(resSettings.data);
      if (resSeo.ok && resSeo.data) setSeoSettings(resSeo.data);
    } catch (err) {
  ...
  useEffect(() => {
    const verifyAuth = async () => {
      ...
      try {
        const { ok, data } = await safeJsonFetch<{ authenticated: boolean; user?: AdminUser }>('/api/auth/verify', {
    ...
    verifyAuth();
  }, [authToken, safeJsonFetch]);

  // Login
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const normalizedEmail = (email || '').trim().toLowerCase();
      const { ok, data, error } = await safeJsonFetch<{ success: boolean; token?: string; user?: AdminUser; error?: string }>('/api/auth/login', {
        method: 'POST',
      ...
      return { success: false, error: data?.error || error || 'Invalid admin credentials' };
    } catch (err: any) {
  ...
  const changeAdminPassword = async (currentPassword: string, newPassword: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const { ok, data, error } = await safeJsonFetch<{ success: boolean; error?: string }>('/api/auth/password', {
        method: 'POST',
        headers: getAuthHeaders(),
