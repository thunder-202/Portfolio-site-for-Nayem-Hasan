...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@nayemhasan.com';
let adminPassword = process.env.ADMIN_PASSWORD || 'nayem2026!';

// Active session tokens
const validSessions = new Map<string, { email: string; name: string; role: string; expiresAt: number }>();

function generateToken(email: string, name: string, role: string): string {
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (cleanEmail === cleanAdminEmail && password === adminPassword) {
    const token = generateToken(cleanEmail, 'Nayem Hasan', 'Super Admin');
  ...
  // Failed login
  addLog('Failed Admin Login Attempt', 'Security', `Invalid attempt for ${cleanEmail}`, 'Security Guard', 'warning');
